...
from SANTO_model import Model
from SANTO_data import intersect, combine_training_data, STDataset
...
def santo(src, tgt, args):
    ...
    plt.legend()
    if args.mode != 'stitch' or args.mode != 'align':
        plt.title("Visualization of source and target slices")
    else:
    ...
    aligned_src_cor = np.dot(aligned_src_cor, fine_R_ab.T) + fine_T_ab
    plt.scatter(tgt_cor[:, 0], tgt_cor[:, 1], c='b', label='target', alpha=0.2,s=0.8)
    plt.scatter(src_cor[:, 0], src_cor[:, 1], c='g', label='source', alpha=0.2,s=0.8)
